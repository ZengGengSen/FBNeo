...
ifeq ($(shell echo ^^),^)
  ...
  coreutils = $(shell where coreutils)
  ifeq ($(coreutils),)
	mkdir   = @$(coreutils) mkdir -p $1
	copy    = @$(coreutils) cp $1 $2
	rcopy   = @$(coreutils) cp -R $1 $2
	delete  = $(info Deleting $1 ...) @$(coreutils) rm -f $1
  else 
...
compiler.system.c   = $(compiler.system) $(flags.c)
compiler.system.cpp = $(compiler.system) $(flags.cpp)
...
ifeq ($(compilers),)
  $(error unknown compiler, please specify manually. compiler and compiler.system)
endif

ifeq ($(compiler),)
  compiler := $(firstword $(compilers))
endif

ifeq ($(compiler.system),)
compiler.system := $(firstword $(compilers))
endif

ifeq ($(perl),)
  perl := $(call which,perl)
  ifeq ($(perl),)
	$(error perl not found, please specify manually.)
  endif
endif
...
ifeq ($(cl),true)
  ...
  flags.c      = -x c -std=gnu99
  flags.cpp    = -x c++ -std=gnu++11
...
ifeq ($(platform),windows)
  ...
  ifeq ($(cl),true)
    ...
    options += $(if $(filter-out true,$(threaded)),,-mthreads)
  else
...
nall.include:
	...
	$(foreach n,$(sort $(call unique,$(flags))),$(if $(filter -I%,$n),$(info $([space]) $n)))
